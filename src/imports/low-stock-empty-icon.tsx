...
export function LowStockEmptyIcon({ className }: { className?: string }) {
  const id = useId();
  const clipId = `clip0-lowstock-empty-${id.replace(/:/g, "")}`;
  const maskId = `mask0-lowstock-empty-${id.replace(/:/g, "")}`;
